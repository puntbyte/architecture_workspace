...
void main() {
  group('NamingPatternRule', () {
    ...
    Future<List<Diagnostic>> runLint({
      ...
      // Note: resolveFile creates its own context, so relative paths in config
      // must match what the analyzer sees (usually file URI or package URI if pubspec exists).
      // For this test, we assume FileResolver handles absolute paths correctly.
    ...
    test('should report error when class name matches NONE of multiple patterns', () async {
      ...
      // FIX: Check correctionMessage for the patterns
      expect(errors.first.correctionMessage, contains('{{name}}Cubit'));
      expect(errors.first.correctionMessage, contains('{{name}}Bloc'));
    });
