...
mixin InheritanceLogic {
  /// Checks if the [element] satisfies the 'required' constraints of [rule].
  ...
  bool satisfiesRule(
    ...
    if (rule.required.isEmpty) return true;
  ...
  String? findComponentIdByInheritance(
    ...
    // FIX: Use allSupertypes to support transitive inheritance (A implements B, B extends C -> A
    // is C)
    ...
    for (final rule in config.inheritances) {
      ...
      if (matchesRule && rule.onIds.isNotEmpty) return rule.onIds.first;
    }
  ...
  bool matchesDefinition(
    ...
    if (def.types.isNotEmpty) {
      if (def.types.contains(element.name)) {
        if (def.imports.isNotEmpty) {
          ...
          final matched = def.imports.any((i) => uri == i || uri.startsWith(i));
    ...
    if (def.component != null) {
      ...
      if (componentContext != null) {
        if (componentContext.matchesReference(def.component!)) return true;
      }
    }
  ...
  AstNode? getNodeForType(ClassDeclaration node, InterfaceType type) {
    if (node.extendsClause?.superclass.type == type) return node.extendsClause!.superclass;
