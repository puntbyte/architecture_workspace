// lib/src/config/schema/exception_policy.dart

import 'package:architecture_lints/src/schema/constants/config_keys.dart';
import 'package:architecture_lints/src/schema/constraints/exception_constraint.dart';
import 'package:architecture_lints/src/schema/descriptors/exception_conversion.dart';
...
class ExceptionPolicy {
  ...
  factory ExceptionPolicy.fromMap(Map<dynamic, dynamic> map) {
  ...
  static List<ExceptionConstraint> _parseList(dynamic value) {
    if (value is List) return value.whereType<Map>().map(ExceptionConstraint.fromMap).toList();

    // Handle single object shorthand
    if (value is Map) return [ExceptionConstraint.fromMap(value)];

    return [];
  }

  static List<ExceptionConversion> _parseConversions(dynamic value) {
    if (value is List) return value.whereType<Map>().map(ExceptionConversion.fromMap).toList();

    return [];
  }

  /// Parses the 'exceptions' list.
  static List<ExceptionPolicy> parseList(List<Map<String, dynamic>> list) =>
      list.map(ExceptionPolicy.fromMap).toList();
}
