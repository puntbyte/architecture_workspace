...
void main() {
  group('JsonMapExtension', () {
    ...
    group('getBool', () {
      ...
      test('should return provided orElse value when key does not exist', () {
        final source = {'other': 'value'};
        expect(source.getBool('enabled', true), isTrue);
      });

      test('should return default false when value is not a boolean', () {
        final source = {'enabled': 'not_a_bool'};
        expect(source.getBool('enabled'), isFalse);
      });

      test('should return provided orElse value when value is not a boolean', () {
        final source = {'enabled': 123};
        expect(source.getBool('enabled', true), isTrue);
      });

      test('should return default false when value is null', () {
